...
import 'package:fd_fitness_app/presentation/screens/update_weight/bloc/update_weight_bloc.dart';
import 'package:flutter/material.dart';
import 'package:flutter_bloc/flutter_bloc.dart';

import '../update_weight_screen.dart';

class UpdateWeightComponent extends StatefulWidget {
  const UpdateWeightComponent({Key? key}) : super(key: key);

  @override
  _UpdateWeightComponentState createState() => _UpdateWeightComponentState();
}

class _UpdateWeightComponentState extends State<UpdateWeightComponent> with FlToastMixin<UpdateWeightComponent> {
  final FocusNode _focusNode = FocusNode();

  UpdateWeightBloc get _bloc => context.read<UpdateWeightBloc>();

  @override
  Widget build(BuildContext context) {
    final UpdateWeightState state = context.watch<UpdateWeightBloc>().state;
    final bool isLoading = state is LoadingState;
    return Scaffold(
      body: BlocConsumer<UpdateWeightBloc, UpdateWeightState>(
        listener: (BuildContext context, UpdateWeightState state) {
          if (state is SucceedSavingDataState) {
            if (_bloc.type == UpdateWeightType.endOfPhase) {
        ...
        builder: (BuildContext context, UpdateWeightState state) => FullScreenLoaderWidget(
          isLoading: isLoading,
